...
final class ContextifyQueryServiceTests: XCTestCase {
  ...
  // MARK: - ct-835: Cloud push export includes summaries, usage, tool_invocations, transcript_metadata

  /// ct-835: exportForCloudPush includes summaries from timeline_cache scoped to batch entries.
  func testExportForCloudPush_includesSummaries() throws {
    let tempDir = FileManager.default.temporaryDirectory
      .appendingPathComponent("contextify-push-summaries-\(UUID().uuidString)")
    try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
    defer { try? FileManager.default.removeItem(at: tempDir) }
    ...
    try pool.write { db in
      try db.execute(sql: """
        INSERT INTO projects (id, name, root_path, cloud_sync_enabled, created_at, updated_at, last_viewed_ts)
        VALUES ('proj-1', 'Test', '/test/push-summaries', 1, 0, 0, 0)
      """)
      try db.execute(sql: """
        INSERT INTO transcripts (id, project_id, file_path, provider,
          last_modified, line_count, last_processed_line, parser_version,
      ...
      try db.execute(sql: """
        ...
        VALUES ('entry-1', 'tx-1', 'proj-1', 'claude.code', 'assistant',
          1000, 'test content', 'sha-1', 1, 1000, 1000)
      """)
      try db.execute(sql: """
        ...
        VALUES ('sha-1', 'win-1', 'entry-1', 'test-gen', 'action',
          'Adding tests', 'Added tests', 'past', 1000, 0)
      """)
    }
    ...
    XCTAssertEqual(export.summaries.first?.entryId, "entry-1")
    XCTAssertEqual(export.summaries.first?.presentForm, "Adding tests")
    XCTAssertEqual(export.summaries.first?.pastForm, "Added tests")
    XCTAssertEqual(export.summaries.first?.disposition, "action")
  }

  /// ct-835: exportForCloudPush includes usage records from assistant_usage scoped to batch entries.
  func testExportForCloudPush_includesUsage() throws {
    let tempDir = FileManager.default.temporaryDirectory
      .appendingPathComponent("contextify-push-usage-\(UUID().uuidString)")
    try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
    defer { try? FileManager.default.removeItem(at: tempDir) }
    ...
    try pool.write { db in
      try db.execute(sql: """
        INSERT INTO projects (id, name, root_path, cloud_sync_enabled, created_at, updated_at, last_viewed_ts)
        VALUES ('proj-1', 'Test', '/test/push-usage', 1, 0, 0, 0)
      """)
      try db.execute(sql: """
        INSERT INTO transcripts (id, project_id, file_path, provider,
          last_modified, line_count, last_processed_line, parser_version,
      ...
      try db.execute(sql: """
        INSERT INTO transcript_entries (id, transcript_id, project_id, provider, kind,
          timestamp, content, content_sha256, display_in_timeline, created_at, updated_at)
        VALUES ('entry-1', 'tx-1', 'proj-1', 'claude.code', 'assistant',
          1000, 'test content', 'sha-usage', 1, 1000, 1000)
      """)
      try db.execute(sql: """
        INSERT INTO assistant_usage (entry_id, request_id, model, input_tokens, output_tokens,
          cache_creation_tokens, cache_read_tokens)
        VALUES ('entry-1', 'req-1', 'claude-sonnet-4-6', 500, 1200, 100, 50)
      """)
    }
    ...
    XCTAssertEqual(export.usage.first?.entryId, "entry-1")
    XCTAssertEqual(export.usage.first?.requestId, "req-1")
    XCTAssertEqual(export.usage.first?.model, "claude-sonnet-4-6")
    XCTAssertEqual(export.usage.first?.inputTokens, 500)
    XCTAssertEqual(export.usage.first?.outputTokens, 1200)
    XCTAssertEqual(export.usage.first?.cacheCreationTokens, 100)
    XCTAssertEqual(export.usage.first?.cacheReadTokens, 50)
  }

  /// ct-835: exportForCloudPush includes tool invocations scoped to batch entries.
  func testExportForCloudPush_includesToolInvocations() throws {
    let tempDir = FileManager.default.temporaryDirectory
      .appendingPathComponent("contextify-push-tools-\(UUID().uuidString)")
    try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
    defer { try? FileManager.default.removeItem(at: tempDir) }
    ...
    try pool.write { db in
      try db.execute(sql: """
        INSERT INTO projects (id, name, root_path, cloud_sync_enabled, created_at, updated_at, last_viewed_ts)
        VALUES ('proj-1', 'Test', '/test/push-tools', 1, 0, 0, 0)
      """)
      try db.execute(sql: """
        INSERT INTO transcripts (id, project_id, file_path, provider,
          last_modified, line_count, last_processed_line, parser_version,
      ...
      try db.execute(sql: """
        INSERT INTO transcript_entries (id, transcript_id, project_id, provider, kind,
          timestamp, content, content_sha256, display_in_timeline, created_at, updated_at)
        VALUES ('entry-1', 'tx-1', 'proj-1', 'claude.code', 'assistant',
          1000, 'test content', 'sha-tools', 1, 1000, 1000)
      """)
      try db.execute(sql: """
        ...
        VALUES ('ti-1', 'entry-1', 'tx-1', 'Read', 'read_file', 'completed',
          1000, 1001, '{"path":"/tmp/test.swift"}', 0, 1000, 1001)
      """)
    }
    ...
    XCTAssertEqual(export.toolInvocations.first?.id, "ti-1")
    XCTAssertEqual(export.toolInvocations.first?.toolName, "Read")
    XCTAssertEqual(export.toolInvocations.first?.status, "completed")
    XCTAssertEqual(export.toolInvocations.first?.metadataJson, "{\"path\":\"/tmp/test.swift\"}")
  }

  /// ct-835: exportForCloudPush includes transcript metadata scoped to batch transcripts.
  func testExportForCloudPush_includesTranscriptMetadata() throws {
    let tempDir = FileManager.default.temporaryDirectory
      .appendingPathComponent("contextify-push-metadata-\(UUID().uuidString)")
    try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
    defer { try? FileManager.default.removeItem(at: tempDir) }
    ...
    try pool.write { db in
      try db.execute(sql: """
        INSERT INTO projects (id, name, root_path, cloud_sync_enabled, created_at, updated_at, last_viewed_ts)
        VALUES ('proj-1', 'Test', '/test/push-metadata', 1, 0, 0, 0)
      """)
      try db.execute(sql: """
        INSERT INTO transcripts (id, project_id, file_path, provider,
          last_modified, line_count, last_processed_line, parser_version,
      ...
      try db.execute(sql: """
        INSERT INTO transcript_entries (id, transcript_id, project_id, provider, kind,
          timestamp, content, content_sha256, display_in_timeline, created_at, updated_at)
        VALUES ('entry-1', 'tx-1', 'proj-1', 'claude.code', 'assistant',
          1000, 'test content', 'sha-meta', 1, 1000, 1000)
      """)
      try db.execute(sql: """
        ...
        VALUES ('tx-1', 'proj-1', 'Cloud Push Test Session', 'Testing cloud push wiring',
          '["cloud","testing","sync"]', 0.95, 0, 0, 1000,
          'claude-sonnet-4-6', 1, 1, 'sha-transcript', 10,
    ...
    XCTAssertEqual(export.transcriptMetadata.first?.transcriptId, "tx-1")
    XCTAssertEqual(export.transcriptMetadata.first?.title, "Cloud Push Test Session")
    XCTAssertEqual(export.transcriptMetadata.first?.topics, "[\"cloud\",\"testing\",\"sync\"]")
    XCTAssertEqual(export.transcriptMetadata.first?.confidence, 0.95)
    XCTAssertEqual(export.transcriptMetadata.first?.model, "claude-sonnet-4-6")
  }

  /// ct-835: exportForCloudPush returns empty arrays for new types when no related data exists.
  func testExportForCloudPush_emptyWhenNoRelatedData() throws {
    let tempDir = FileManager.default.temporaryDirectory
      .appendingPathComponent("contextify-push-empty-\(UUID().uuidString)")
    try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
    defer { try? FileManager.default.removeItem(at: tempDir) }
    ...
    try pool.write { db in
      try db.execute(sql: """
        INSERT INTO projects (id, name, root_path, cloud_sync_enabled, created_at, updated_at, last_viewed_ts)
        VALUES ('proj-1', 'Test', '/test/push-empty', 1, 0, 0, 0)
      """)
      try db.execute(sql: """
        INSERT INTO transcripts (id, project_id, file_path, provider,
          last_modified, line_count, last_processed_line, parser_version,
      ...
      try db.execute(sql: """
        INSERT INTO transcript_entries (id, transcript_id, project_id, provider, kind,
          timestamp, content, content_sha256, display_in_timeline, created_at, updated_at)
        VALUES ('entry-1', 'tx-1', 'proj-1', 'claude.code', 'assistant',
          1000, 'test content', 'sha-empty', 1, 1000, 1000)
      """)
    }
    ...
    XCTAssertEqual(export.entries.count, 1, "Should have the entry")
    XCTAssertTrue(export.summaries.isEmpty, "No timeline_cache rows for this entry")
    XCTAssertTrue(export.usage.isEmpty, "No assistant_usage rows for this entry")
    XCTAssertTrue(export.toolInvocations.isEmpty, "No tool_invocations rows for this entry")
    XCTAssertTrue(export.transcriptMetadata.isEmpty, "No transcript_metadata rows for this transcript")
  }

  /// ct-835: exportForCloudPush does not include data from entries outside the batch (cloud_sync_enabled=0).
  func testExportForCloudPush_scopesToBatchOnly() throws {
    let tempDir = FileManager.default.temporaryDirectory
      .appendingPathComponent("contextify-push-scope-\(UUID().uuidString)")
    try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
    defer { try? FileManager.default.removeItem(at: tempDir) }
    ...
    try pool.write { db in
      // Project with sync enabled
      try db.execute(sql: """
        INSERT INTO projects (id, name, root_path, cloud_sync_enabled, created_at, updated_at, last_viewed_ts)
        VALUES ('proj-sync', 'Synced', '/test/synced', 1, 0, 0, 0)
      """)
      // Project with sync disabled
      try db.execute(sql: """
        INSERT INTO projects (id, name, root_path, cloud_sync_enabled, created_at, updated_at, last_viewed_ts)
        VALUES ('proj-nosync', 'NoSync', '/test/nosync', 0, 0, 0, 0)
      """)
      // Transcript + entry for synced project
      try db.execute(sql: """
        INSERT INTO transcripts (id, project_id, file_path, provider,
          last_modified, line_count, last_processed_line, parser_version,
          status, ingest_state, created_at, updated_at)
        VALUES ('tx-sync', 'proj-sync', '/test/sync.jsonl', 'claude.code',
          100, 10, 10, 1, 'active', 'complete', 100, 100)
      """)
      try db.execute(sql: """
        INSERT INTO transcript_entries (id, transcript_id, project_id, provider, kind,
          timestamp, content, content_sha256, display_in_timeline, created_at, updated_at)
        VALUES ('entry-sync', 'tx-sync', 'proj-sync', 'claude.code', 'assistant',
          1000, 'synced content', 'sha-sync', 1, 1000, 1000)
      """)
      // Transcript + entry for non-synced project
      try db.execute(sql: """
        INSERT INTO transcripts (id, project_id, file_path, provider,
          last_modified, line_count, last_processed_line, parser_version,
          status, ingest_state, created_at, updated_at)
        VALUES ('tx-nosync', 'proj-nosync', '/test/nosync.jsonl', 'claude.code',
          100, 10, 10, 1, 'active', 'complete', 100, 100)
      """)
      try db.execute(sql: """
        INSERT INTO transcript_entries (id, transcript_id, project_id, provider, kind,
          timestamp, content, content_sha256, display_in_timeline, created_at, updated_at)
        VALUES ('entry-nosync', 'tx-nosync', 'proj-nosync', 'claude.code', 'assistant',
          1000, 'not synced', 'sha-nosync', 1, 1000, 1000)
      """)
      // Add usage to BOTH entries
      try db.execute(sql: """
        INSERT INTO assistant_usage (entry_id, request_id, model, input_tokens, output_tokens,
          cache_creation_tokens, cache_read_tokens)
        VALUES ('entry-sync', 'req-sync', 'claude-sonnet-4-6', 100, 200, 0, 0)
      """)
      try db.execute(sql: """
        INSERT INTO assistant_usage (entry_id, request_id, model, input_tokens, output_tokens,
          cache_creation_tokens, cache_read_tokens)
        VALUES ('entry-nosync', 'req-nosync', 'claude-sonnet-4-6', 300, 400, 0, 0)
      """)
    }
    ...
    // Only synced entry should be in the batch
    XCTAssertEqual(export.entries.count, 1)
    XCTAssertEqual(export.entries.first?.id, "entry-sync")

    // Only usage from the synced entry should appear
    XCTAssertEqual(export.usage.count, 1)
