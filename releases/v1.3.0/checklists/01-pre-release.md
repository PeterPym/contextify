...
- [ ] Check if this version was ever submitted to App Store:
  ```bash
  grep -A5 '"1.3.0"' releases/manifest.json | grep -q '"submitted"' && echo "Was submitted" || echo "Never submitted"
  ```
...
- [ ] Save to: `releases/v1.3.0/assets/release-notes-draft.md`
...
```bash
./scripts/release/validate-pre-release.sh 1.3.0
```
