...
```bash
./scripts/release/build.sh 1.0.5
```
...
- Archives to `build/archives/v1.0.5/`
- Updates `release.json` with results

### Verify Build Output

- [ ] Check archive directory: `ls -la build/archives/v1.0.5/`
- [ ] Verify App Store archive: `build/archives/v1.0.5/appstore/Contextify.xcarchive`
- [ ] Verify .pkg: `build/archives/v1.0.5/appstore/Contextify-1.0.5.pkg`
- [ ] Verify DMG: `build/archives/v1.0.5/dmg/Contextify-1.0.5.dmg`
...
```bash
python3 scripts/release/release.py --version 1.0.5 --yes
./scripts/sparkle/sign.sh dist/Contextify-1.0.5.dmg
```

- [ ] DMG created: `dist/Contextify-1.0.5.dmg`
...
mkdir -p build/archives/v1.0.5/appstore
cp -R build/Contextify.xcarchive build/archives/v1.0.5/appstore/Contextify.xcarchive
cp build/appstore/Contextify.pkg build/archives/v1.0.5/appstore/Contextify-1.0.5.pkg
mkdir -p build/archives/v1.0.5/dmg
cp dist/Contextify-1.0.5.dmg build/archives/v1.0.5/dmg/
```
...
```bash
./scripts/release/validate-build.sh 1.0.5
```
