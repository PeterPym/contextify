...
- [ ] New release notes page created? `website/release-notes/1.5.0.html`
...
cat dist/Contextify-1.5.0.dmg.sha256

# Download and hash the public DMG
curl -sL "https://github.com/PeterPym/contextify/releases/download/v1.5.0/Contextify-1.5.0.dmg" | shasum -a 256
```
...
curl -sL "https://github.com/PeterPym/contextify/releases/download/v1.5.0/Contextify-1.5.0.dmg" -o /tmp/verify.dmg
...
curl -s https://contextify.sh/appcast.xml | grep -A5 "Version 1.5.0" | grep "sparkle:version"
...
- [ ] Ensure `releases/v1.5.0/` is complete
...
- [ ] DMG shipped:
  ```bash
  ./scripts/release/mark-shipped.sh 1.5.0 --dmg
  ```
- [ ] App Store approved:
  ```bash
  ./scripts/release/mark-shipped.sh 1.5.0 --appstore --build <BUILD_NUMBER>
  ```
<!-- IF:linux -->
- [ ] Linux shipped:
  ```bash
  ./scripts/release/mark-shipped.sh 1.5.0 --linux
  ```
<!-- ENDIF:linux -->
- [ ] Verify manifest is correct: `cat releases/manifest.json | python3 -m json.tool | grep -A20 '"1.5.0"'`

### Git Housekeeping
- [ ] Release tag exists: `git tag -l v1.5.0`
...
**Version:** 1.5.0
**Released:** ____
**DMG:** https://github.com/banagale/contextify/releases/tag/v1.5.0
