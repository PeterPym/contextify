...
```bash
./scripts/release/build.sh 1.2.1
```

This script:
- Validates tests pass
- Builds DMG (Release, signed, notarized)
- Archives to `build/archives/v1.2.1/`
- Updates `release.json` with results

### Verify Build Output

- [ ] Check archive directory: `ls -la build/archives/v1.2.1/`
- [ ] Verify DMG: `build/archives/v1.2.1/dmg/Contextify-1.2.1.dmg`
...
```bash
python3 scripts/release/release.py --version 1.2.1 --yes
./scripts/sparkle/sign.sh dist/Contextify-1.2.1.dmg
```

- [ ] DMG created: `dist/Contextify-1.2.1.dmg`
...
mkdir -p build/archives/v1.2.1/dmg
cp dist/Contextify-1.2.1.dmg build/archives/v1.2.1/dmg/
```
...
```bash
./scripts/release/validate-build.sh 1.2.1
```
