...
# Priority: explicit path > custom location > container Documents > container App Support > default
discover_database_path() {
    local dist_mode="${CONTEXTIFY_DIST:-}"

    # Method 1: Check UserDefaults for custom location FIRST
    # This takes priority because onboarding wizard sets custom location for App Store builds
    ...
    # Method 2: Container Documents (common App Store location from onboarding)
    local container_docs="$HOME/Library/Containers/sh.contextify.Contextify/Data/Documents/Contextify/$DB_NAME"
    if [ -f "$container_docs" ]; then
        echo "$container_docs"
        return 0
    fi

    # Method 3: App Store sandbox container Application Support
    local sandbox_db="$HOME/Library/Containers/sh.contextify.Contextify/Data/Library/Application Support/Contextify/$DB_NAME"
    if [ "$dist_mode" = "appstore" ] || [ -f "$sandbox_db" ]; then
        echo "$sandbox_db"
        return 0
    fi

    # Method 4: Default location (DMG builds)
