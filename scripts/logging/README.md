...
| `analyze-validation-failures.sh` | Extract and analyze LLM validation rejections | Tune bridging lexicon, analyze leakage patterns |
...
## Pattern 9: LLM Validation Failure Analysis

**When to use:**
- Timeline summaries are being rejected by validation
- Need to understand which tokens are leaking into summaries
- Tuning the `bridgingLexicon` to reduce false positives
- Analyzing patterns in validation rejections over time

**Why this is run:**
Timeline summaries are generated by the on-device LLM and validated before being saved. Validation can reject summaries for two reasons:
1. **Excessive leakage:** Too many tokens from the input message appear in the summary (indicates prompt contamination)
2. **Prompt example contamination:** Summary contains exact phrases from the prompt examples

When validation is too strict, legitimate summaries get rejected and fail to generate. When too lenient, contaminated summaries (hallucinations, prompt copying) get through. This script helps tune the threshold and identify which tokens should be in the `bridgingLexicon` (allowed vocabulary that doesn't count as leakage).

**How it works:**
Scans log files for `[VALIDATION-REJECT]` entries, extracts the leaked token list, confidence score, and timestamp, then generates a CSV report with frequency analysis.

**Usage:**
```bash
# Analyze all transcript queue monitor logs
./scripts/logging/analyze-validation-failures.sh

# Analyze specific logs
./scripts/logging/analyze-validation-failures.sh "/tmp/my-logs-*.log"

# Custom output location
./scripts/logging/analyze-validation-failures.sh "/tmp/transcript-*.log" output.csv
```

**Output:**
```
═══════════════════════════════════════════════════════════════
  Timeline Summarization Validation Failure Analysis
═══════════════════════════════════════════════════════════════
Log pattern: /tmp/transcript-queue-monitor-*.log
Output file: /tmp/failed-summaries-20251123-090618.csv

Found 1 failed summarization attempts

Leakage count distribution:
   6 tokens:   1 occurrences

Most common leaked tokens (top 20):
    1: revised
    1: numbering
    1: item
    1: inconsistencies
...
═══════════════════════════════════════════════════════════════
CSV written to: /tmp/failed-summaries-20251123-090618.csv
```

**CSV format:**
```
timestamp,leaked_count,confidence,leaked_tokens
"2025-11-23 08:18:17.234",6,0.750000,"numbering; identified; item; inconsistencies; revised; code"
```

**Analysis workflow:**
1. **Run the script** to generate the CSV
2. **Review "Most common leaked tokens"** - these appear frequently across rejections
3. **Categorize tokens:**
   - **Domain-specific** (numbering, item, inconsistencies) - properly leaked, should count
   - **Standard vocabulary** (identified, revised, code) - should be in `bridgingLexicon`
4. **Update bridging lexicon** in `Contextify/Contextify/FoundationLLM.swift`:
   ```swift
   var bridgingLexicon: Set<String> {
       [
           // Assistant names and common verbs
           "claude", "code", "codex", "identified", "revised", ...
       ]
   }
   ```
5. **Rebuild and retest** to verify rejections decrease

**Common patterns:**
- **Assistant name components** (claude, code, codex) - always add to lexicon
- **Common verbs** (identified, explained, analyzed) - add if standard summary vocabulary
- **Action verbs** (revised, updated, changed) - add if commonly used for summaries
- **Domain terms** (numbering, item, inconsistencies) - leave as leaked tokens (correct)

**Thresholds:**
Current validation logic (FoundationLLM.swift:1806):
```swift
let excessiveLeakage = leaked.count > 6  // Rejects 7+ leaked tokens
```

If too many legitimate summaries fail, consider:
- Adding common vocabulary to `bridgingLexicon` (preferred)
- Raising threshold (last resort - increases risk of contamination)

**Exit code:** Always 0 (script reports results, doesn't fail)

**Next steps:**
- If rejection rate too high → Add frequent tokens to bridging lexicon
- If rejection rate too low → Check for contamination in summaries
- Monitor with: `log stream --predicate 'subsystem BEGINSWITH "dev.contextify"' --level debug | grep VALIDATION`
