...
install_zsh() {
  ...
  cat >>"$rc" <<EOF2
$MARK
# Contextify keybinding: Ctrl-X Ctrl-K
...
function _contextify_compose() {
  ...
  local b64_enc="\$(/usr/bin/python3 -c 'import sys, urllib.parse as u; print(u.quote(sys.stdin.read()))' <<<"\$b64_raw")"
  /usr/bin/open "${SCHEME}://compose?title=Compose&text64=\${b64_enc}"
  zle redisplay
}
zle -N _contextify_compose
bindkey -M emacs '^X^K' _contextify_compose
bindkey -M vicmd '^X^K' _contextify_compose
bindkey -M viins '^X^K' _contextify_compose
$ENDMARK
EOF2
