...
```
scripts/qa/
├── run-all-tests.sh              # Main test orchestrator (sequential)
├── lib/
│   ├── common.sh                 # Shared utilities (log capture, DB queries, wait helpers)
...
**Main orchestrator:** `scripts/qa/run-all-tests.sh`
...
### QA-01: App Launch & Startup

**ID:** QA-01
**Duration:** ~20 seconds
**Prerequisites:** App not running, database exists

**Test Steps:**

1. Kill any running instances of Contextify
2. Start log capture
3. Launch app via `launch_app` (uses `wait_for_log_pattern` internally)
4. Validate startup markers in logs
5. Verify database connection and initial project load
...
# Database (use exact count for known QA project)
...
**Common Failures:**
- App crashes on launch → Check logs for exception messages
- No startup markers → Subsystem mismatch in log capture predicate
- Database locked → Another process holding connection (wait and retry)
...
3. **Accessibility Permission** - For AppleScript keyboard automation
4. **Contextify built** - `.derived/Build/Products/Debug/Contextify.app`
...
- [ ] Implement `lib/common.sh` with `wait_for_log_pattern`
- [ ] Implement `lib/assertions.sh` (exact counts only)
- [ ] Implement `run-all-tests.sh` (sequential orchestrator)
- [ ] Implement QA-03 first (canonical template)
- [ ] Pattern-match QA-01, QA-02, QA-04, QA-05, QA-06 from QA-03
- [ ] Test full suite locally
- [ ] Document usage in `build/docs/testing/`
- [ ] Add entry to `TODOS.md` as P1
