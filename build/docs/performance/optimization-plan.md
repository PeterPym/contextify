# Performance Optimization Plan for Contextify
...
### Phase 1: Measurement and Baselines - COMPLETE
...
1. **Benchmark Harness** - `scripts/benchmarks/run-perf-suite.sh`
   ...
   - Automatic history tracking in `build/docs/performance/benchmark-history.md`

2. **Baseline Established**
   - Startup: 43ms (cold)
   - Ingest rate: 237 lines/sec (baseline)
   - Peak memory: 864MB
   - Corpus: 360,891 lines across 2145 transcripts

3. **Profiling Analysis** - `build/docs/performance/ingest-optimization-analysis.md`
   - Hotspot identified: HooverEngine.commitBatch (90% of time)
   - Root cause: SQLite disk I/O for B-tree page reads
   - 4 bottlenecks identified with fix strategies

**Linux Engine Impact:** Harness is portable; ingest benchmarks apply directly.
...
1. ✅ **Batch Parent Validation** (Shared) - commit 972adb39
   - Changed O(n) per-entry queries to single batch query
   - Added same-batch parent link support
   - Added SQLite parameter limit chunking (groups of 500)
   - **Result:** +3% improvement (237 → 244 lines/sec)

2. ✅ **Batch Size Tuning** (Shared)
   - Tested batch size 2000 vs default 1000
...
7. ⏳ **TranscriptWatcher Debounce Tuning** (Mac)
   - Experiment: 100ms, 200ms, 300ms

8. ⏳ **LLM Queue Prefetch** (Mac)
