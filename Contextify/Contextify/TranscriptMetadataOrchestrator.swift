...
actor TranscriptMetadataOrchestrator {
  ...
  private func generateMetadata(
    ...
    // Check if this is a metadata-only transcript (entryCount == 0)
    if session.entryCount == 0 {
      log.info("Detected metadata-only transcript, classifying type...")
      let metadataType = try await orchestrator.classifyMetadataTranscript(transcriptId)

      switch metadataType {
      case .emptySnapshots(let lineCount):
        log.info("Metadata type: Empty snapshots - using simple title")

        let metadata = TranscriptMetadata(
          title: "Session Record",
          description: "\(lineCount) checkpoints",
          topics: ["metadata"],
          confidence: 1.0,
          ...
          transcriptSHA256: try FileFacts.stableSha256(url: session.fileURL),
          messageCount: 0,
      ...
      case .fileTracking(let fileCount, let topFiles):
        log.info("Metadata type: File tracking (\(fileCount) files) - using LLM to summarize")

        // Build file list prompt
        let fileList = topFiles.prefix(10).map { path in
          let filename = URL(fileURLWithPath: path).lastPathComponent
          return "- \(filename)"
        }.joined(separator: "\n")

        let fileTrackingPrompt = """
        Analyze this list of tracked files. Generate a concise title (≤40 chars) describing the work area or feature being developed:

        Files tracked (\(fileCount) total):
        \(fileList)
        \(fileCount > 10 ? "... and \(fileCount - 10) more files" : "")

        Title should describe the feature/area being worked on. Examples: "Projects Feature Development", "Database Layer Refactor", "UI Component Updates"
        """

        // Use LLM to generate title and description for file tracking
        let llmStart = Date()
        let fileTrackingMetadata: TranscriptMetadata
        ...
        do {
          ...
          if #available(macOS 26, *) {
            ...
            let instructions = """
            You are analyzing a file tracking session from a development transcript. Generate a clear, descriptive title and brief description of what files were being worked on.

            Output in JSON format:
            {
              "title": "concise title (40 chars max)",
              "description": "brief description of work area (100 chars max)",
            ...
            fileTrackingMetadata = TranscriptMetadata(
              title: String(guided.title.prefix(40)),
              description: "\(fileCount) files tracked: \(guided.description)",
              topics: guided.topics,
              confidence: 0.9,
              mayContainHallucinations: false,
              needsReview: false,
              generatedAt: Date(),
              model: "foundation-llm",
              promptVersion: currentPromptVersion,
              generatorVersion: currentGeneratorVersion,
              transcriptSHA256: try FileFacts.stableSha256(url: session.fileURL),
              messageCount: 0,
              strategy: "metadata-file-tracking",
              llmCalls: 1,
              latencyMs: Int(llmTime * 1000)
            )
          ...
          log.warning("LLM failed for file tracking, using heuristic")
          await circuitBreaker.recordFailure()

          // Fallback: use first 2 filenames in title
          let firstFiles = topFiles.prefix(2).map { URL(fileURLWithPath: $0).lastPathComponent }
          let titleSuffix = fileCount > 2 ? " +\(fileCount - 2)" : ""

          fileTrackingMetadata = TranscriptMetadata(
            title: "File Tracking (\(fileCount) files)",
            description: "\(firstFiles.joined(separator: ", "))\(titleSuffix)",
            topics: ["file-tracking"],
            confidence: 0.7,
            mayContainHallucinations: false,
            needsReview: false,
            generatedAt: Date(),
            ...
            transcriptSHA256: try FileFacts.stableSha256(url: session.fileURL),
            messageCount: 0,
            strategy: "metadata-file-tracking-heuristic",
            llmCalls: 0,
            latencyMs: 0
          )
        }

        try await saveToSQL(fileTrackingMetadata, transcriptId: transcriptId, fileURL: session.fileURL, orchestrator: orchestrator)
        return fileTrackingMetadata

      case .sidechainConversation, .mixed:
        log.info("Metadata type: Sidechain or mixed - using standard LLM pipeline")
        // Fall through to standard LLM processing below
      }
    }
