...
public final class TranscriptOrchestrator: @unchecked Sendable {
  ...
  /// Upsert a single transcript within an existing transaction
  /// **CRITICAL:** This method assumes it's being called within a `db.write` transaction.
  /// DO NOT call this directly - use `upsertTranscripts` for normal operations.
  /// This is intended for FastPathIngestionCoordinator batching to avoid nested transactions.
  ///
  /// - Parameters:
  ///   - path: File path to the transcript
  ///   - projectId: Project ID this transcript belongs to
  ///   - db: Active GRDB Database handle from write transaction
  /// - Throws: Database errors or validation errors
  internal func upsertTranscriptInTransaction(
    path: String,
    projectId: String,
    provider: String,
    sessionId: String?,
    ...
    if let ex = existing, let id = ex["id"] as? String {
      ...
      try db.execute(sql: """
        ...
        WHERE id = ?
      """, arguments: [
        path, normalizedPath, pathHash,
        TimeUnits.secondsFromMs(mtimeMs),
        len > 0 ? Int(len) : nil,
        len, mtimeMs, sha, nowSec, id
      ])
      log.debug("[BATCH-UPSERT] Updated transcript: \(id.prefix(8))... [\(pathHash.prefix(8))...]")
    } else {
      // New transcript - insert
      ...
      """, arguments: [
        transcriptId, projectId, path, normalizedPath, pathHash,
        provider, sessionIdArg,
        TimeUnits.secondsFromMs(mtimeMs), len > 0 ? Int(len) : nil, len, mtimeMs, sha,
        0, 0, 1, "active", "partial",
        nowSec, nowSec
      ])
      log.debug("[BATCH-UPSERT] Created transcript: \(transcriptId.prefix(8))... [\(pathHash.prefix(8))...]")
    }
  }
